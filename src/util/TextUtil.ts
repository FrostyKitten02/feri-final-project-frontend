...
export default class TextUtil {
  ...
  static getMonthNumber(
    ...
    startMonth: ProjectMonthDto | undefined,
    currentPage: number,
    ...
    if (!value || !startMonth || !startMonth.date || !startMonth.monthNumber)
      return 0;
    const diff =
      new Date(startMonth.date).getMonth() + 1 - startMonth.monthNumber;
  ...
  static returnWorkpackageLimit = (
    ...
    shownMonths: Array<ProjectMonthDto> | undefined
  ...
  static getWorkStatusColors = (month: ProjectMonthDto): string => {
    if (!month.pmBurnDownRate) return "";
  ...
  static getSpendingStatusColors = (month: ProjectMonthDto): string => {
    if (!month.staffBudgetBurnDownRate) return "";
